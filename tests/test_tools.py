...
class ToolsTests(unittest.TestCase):
    ...
    def test_web_fetch_tool_is_registered_and_web_browser_is_not(self):
        from kittychain.tools import get_tool
        ...
        self.assertIsNone(get_tool("web_browser"))
...
""".strip()
    ...
    def test_web_fetch_main_prints_result(self):
        original_execute = web_fetch_module.WebFetchTool.execute
        try:
            web_fetch_module.WebFetchTool.execute = lambda self, url, prompt="", timeout=20: f"summary:{url}:{prompt}:{timeout}"
            buffer = io.StringIO()
            with redirect_stdout(buffer):
                exit_code = web_fetch_module.main("https://example.com")
        finally:
            web_fetch_module.WebFetchTool.execute = original_execute

        self.assertEqual(exit_code, 0)
        self.assertEqual(buffer.getvalue().strip(), "summary:https://example.com::20")

    def test_web_fetch_tool_summarizes_page_text_with_prompt(self):
        class FakeResponse:
            def __init__(self):
                self.url = "https://example.com/risk"
                self.status_code = 200
                self.headers = {"Content-Type": "text/html"}
                self.text = "<html><body><main><h1>Risk page</h1><p>Page body text</p></main></body></html>"
        ...
        tool = web_fetch_module.WebFetchTool()
        tool.bind_agent(FakeAgent())
        try:
            web_fetch_module.requests.get = lambda *args, **kwargs: FakeResponse()
            result = tool.execute(url="https://example.com", prompt="extract counterparties", timeout=20)
        finally:
        ...
        self.assertIn("agent-browser", kwargs["system"])

    def test_web_fetch_tool_strips_think_blocks_from_summary(self):
        class FakeResponse:
            def __init__(self):
                self.url = "https://example.com/risk"
                self.status_code = 200
                self.headers = {"Content-Type": "text/html"}
                self.text = "<html><body><p>Page body text</p></body></html>"
        ...
        try:
            web_fetch_module.requests.get = lambda *args, **kwargs: FakeResponse()
            result = tool.execute(url="https://example.com", prompt="summarize", timeout=20)
        finally:
            web_fetch_module.requests.get = original_get
