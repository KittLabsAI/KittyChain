"""Interactive CLI for KittyChain."""
...
def _load_config() -> Config:
    try:
        ...
        console.print("Run `kittychain --config` to create or repair your model configuration.")
...
def _show_skills(skills, io=None):
    io = io or _build_input_reader(_default_history_path(), lambda: list(_BUILTIN_COMMANDS))
    io.print(Panel(_format_skills(skills), title="KittyChain Skills", border_style="dim"))
...
def _format_skills(skills) -> str:
    if not skills:
        return "No skills loaded from the current KittyChain skill roots"
...
def _build_history_line_metadata(item: _HistoryItem, rendered: str) -> list[dict[str, object]]:
    ...
    assistant_label_present = bool(item.role == "assistant" and lines and lines[0] == "KittyChain")
